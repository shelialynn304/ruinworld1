...
Preferred domain: `https://ruinworld-branchandbone.com`
...
|---|---|---|---|---|---|---|
| `index.html` | None | `https://ruinworld-branchandbone.com/` | Yes (no non-canonical absolute internal URLs present) | Yes (added JSON-LD `url` set to canonical) | Yes (documented for redirect handling) | Yes (see `redirect-audit.md`) |
...
- `index.html`: `og:url` now matches canonical exactly (`https://ruinworld-branchandbone.com/`).
...
- Homepage listed as `https://ruinworld-branchandbone.com/` (no `/index.html` duplicate entry).
